...
```
configs/
  base.config              # Shared build config (target, kernel, WiFi, ZRAM, core packages)
  variants/
    ap.config              # AP mode: mesh + minimal LuCI
    router.config          # Router mode: firewall + full LuCI
files/
  ap/
...
1. Create `configs/variants/server.config` with the desired packages
2. Create `files/server/etc/uci-defaults/` with any first-boot scripts
...
The final `.config` is generated by concatenating `configs/base.config` + `configs/variants/<mode>.config`. Settings in the variant config override the base. To customize packages for an existing variant, edit its config file under `configs/variants/`.
